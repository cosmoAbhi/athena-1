...
//  Real rho_pol = std::sqrt(std::pow(pcoord->x1v(i),2.) + std::pow(pcoord->x2v(j),2.));
  ...
  Real sinphi = sin(pcoord->x3v(k));
	if (r<R){
	  ...
	  interp_lag4(tov->data[itov_rho], tov->data[itov_rsch], tov->npts, r,
		      &rho_kji, &dummy,&dummy);
	  // Pressure from EOS
	  //TODO (SB) general EOS call 
  ...
  for (int k=kl; k<=ku+1; ++k) {
	...
	if (r<R){
	  ...
	  if (r == 0.) {
	    ...
	    interp_lag4(tov->data[itov_lapse], tov->data[itov_rsch], tov->npts, r,
			&lapse_kji, &d_lapse_dr_kji,  &dummy);
	    interp_lag4(tov->data[itov_psi4], tov->data[itov_rsch], tov->npts, r,
			&psi4_kji, &d_psi4_dr_kji,  &dummy);
	  }	
	} else {
	  // Exterior schw. metric, lapse and conf.fact.
	  lapse_kji = pow((1.-2.0*M/r), 0.5); 
	  psi4_kji =  pow((1.-2.0*M/r),-1.0); 
	}
